  ...
  const calculateStats = (transactions: Transaction[]) => {
    ...
    setExpenseBreakdown(
      Object.entries(breakdown).map(([name, value]) => ({
        name: EXPENSE_CATEGORIES.find(c => c.value === name)?.label || 'Uncategorized',
    ...
    supabase
      ...
      .then(({ data: profile }) => {
        ...
        expenseBreakdown.forEach(cat => {
          doc.text(`  ${cat.name}: PHP ${cat.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, margin + 5, yPos);
          yPos += 6;
          if (yPos > 250) {
            doc.addPage();
            yPos = margin;
        ...
        const otherExpenses = totalExpenses - expenseBreakdown.reduce((sum, cat) => sum + cat.value, 0);
        if (otherExpenses > 0) {
