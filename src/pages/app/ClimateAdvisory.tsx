...
const ClimateAdvisory = () => {
    ...
    try {
      ...
      if (forecastData) {
        ...
        if (forecastList && forecastList.length > 0) {
          ...
          console.log('Forecast list set with', dailyForecasts.length, 'daily forecasts');
        } else {
          console.warn('No valid forecast list found in response:', Object.keys(forecastData || {}));
        }
      }
    } catch (error: any) {
  ...
  const generateAdvisories = (): Array<{ type: string; title: string; message: string; icon: string }> => {
    ...
    const nextRain = forecast.list?.find((day: ForecastDay | any) => {
      const dayRain = day.rain || day.precipitation;
      if (!dayRain) return false;
      if (typeof dayRain === 'number') return dayRain > 0;
      if (typeof dayRain === 'object' && dayRain['3h']) return dayRain['3h'] > 0;
      return false;
    });
    ...
    if (nextRain) {
      ...
      const rainAmount = nextRain.rain || (nextRain as any).precipitation;
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-white/60 text-sm mb-4">
                Please configure VITE_AGROMONITORING_API_KEY in your environment variables
              </p>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground text-xs">
                {isApiConfigured() 
                  ...
                  : "Please configure VITE_AGROMONITORING_API_KEY in your .env file"}
              </p>
            </div>
          ) : (
