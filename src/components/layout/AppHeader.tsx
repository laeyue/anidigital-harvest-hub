...
const AppHeader = ({ onMenuClick }: AppHeaderProps) => {
  ...
  const [searchResults, setSearchResults] = useState<{ products: SearchProduct[]; farmers: SearchFarmer[]; shops: SearchShop[] }>({ products: [], farmers: [], shops: [] });
  ...
  }, [user, loadProfile, loadNotifications, loadUnreadMessages]);
  ...
  const performSearch = async (query: string) => {
    try {
      ...
      setSearchResults({
        products: (productsData || []).map((p: { profiles?: { name: string } | { name: string }[] }) => ({
          ...p,
          profiles: Array.isArray(p.profiles) ? p.profiles[0] : p.profiles,
        })) as SearchProduct[],
        farmers: (farmersData || []) as SearchFarmer[],
        shops: (shopsData || []).map((s: { profiles?: { name: string } | { name: string }[] }) => ({
          ...s,
  ...
  }, [notifications, unreadMessagesCount, updateTotalUnreadCount]);
  ...
  const handleSearch = (e?: React.FormEvent | React.KeyboardEvent) => {
    ...
    setIsSearchOpen(false);
    if (searchQuery.trim()) {
      router.push(`/app/marketplace?search=${encodeURIComponent(searchQuery.trim())}`);
    } else {
  ...
  const handleResultClick = (type: 'product' | 'farmer' | 'shop', id: string, name: string) => {
    ...
    if (type === 'shop') {
      ...
      router.push(`/app/marketplace?search=${encodeURIComponent(name)}`);
    }
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem asChild>
              <Link href="/app/finances">Financial Summary</Link>
            </DropdownMenuItem>
            <DropdownMenuItem asChild>
