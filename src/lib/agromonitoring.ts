...
// To set this up, add VITE_AGROMONITORING_API_KEY to your .env file
const getApiKey = () => {
  return import.meta.env.VITE_AGROMONITORING_API_KEY || '';
};
...
export const deletePolygon = async (polygonId: string): Promise<boolean> => {
  ...
  try {
    ...
    if (!polygonExists) {
      console.log('Polygon does not exist, skipping deletion:', polygonId);
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        console.log('Polygon not found during deletion (may have been already deleted):', polygonId);
        return true; // Return true because the goal is achieved
      }
      console.error('Error deleting polygon:', await response.text());
      return false;
    }
...
export const createPolygon = async (location: string, polygonName: string): Promise<string | null> => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Error creating polygon:', await response.text());
      return null;
    }
...
export const getCurrentWeather = async (polygonId: string, forceRefresh = false) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Error fetching current weather:', await response.text());
...
export const getHourlyForecast = async (polygonId: string) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Error fetching hourly forecast:', await response.text());
      return null;
    }
...
export const getPolygonDetails = async (polygonId: string) => {
  ...
  try {
    ...
    console.error('Error fetching polygon details:', error);
    return null;
  }
};
...
export const getDailyForecast = async (polygonId: string, forceRefresh = false) => {
  ...
  try {
    ...
    if (!polygon || !polygon.geo_json || !polygon.geo_json.geometry) {
      console.error('Could not get polygon details');
    ...
    if (!response.ok) {
      ...
      console.error('Error fetching daily forecast:', response.status, errorText);
      // Return cached data if API fails
      return getCachedData(cacheKey);
    }
...
export const getHistoricalWeather = async (polygonId: string, start: number, end: number) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Error fetching historical weather:', await response.text());
      return null;
    }
